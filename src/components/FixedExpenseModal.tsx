...
export const FixedExpenseModal: React.FC<FixedExpenseModalProps> = ({
  ...
  const handleOpenEditForm = (rule: FixedExpense) => {
    ...
    setErrorMsg('');
    setEditingRule(rule);
    setSelectedPaymentMethodId(rule.paymentMethodId || 'pay-cash');
    setPanel('form');
  };
  ...
  return (
    <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 overflow-hidden overflow-x-hidden">
      ...
      <motion.div
        ...
        className="relative w-full max-w-md bg-[var(--color-bg)] border-t-4 border-[var(--color-active-border)] sm:border-4 sm:rounded-[36px] rounded-t-[36px] p-6 shadow-2xl z-10 overflow-hidden overflow-x-hidden flex flex-col justify-between max-h-[92vh] sm:max-h-[90vh]"
      >
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                      ...
                      {Object.keys(categories).map((catName) => {
                        ...
                        return (
                          <div key={catName} className="relative">
                            ...
                            >
                              <div 
                              className={`p-1 rounded-xl mb-1 shrink-0 transition-colors ${
                                isDeleteMode ? 'bg-red-100 text-red-600' : isSelected ? 'bg-white/20' : (catInfo.bgColor.startsWith('bg-') ? catInfo.bgColor : '')
                              }`}
                    ...
                    {isAddingCustomCat && onAddCategory && (
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          ...
                          <div className={showAllIcons ? "flex flex-wrap gap-1.5 max-h-[140px] overflow-y-auto p-1 bg-neutral-50/50 rounded-xl border border-neutral-100" : "flex gap-1.5 overflow-x-auto pb-1 scrollbar-thin"}>
                            ...
                            {!showAllIcons && (
                              <button
                                ...
                                className="flex flex-col items-center shrink-0 p-1.5 rounded-xl border-2 border-dashed border-neutral-300 bg-neutral-50 text-neutral-500 hover:border-black hover:text-black hover:bg-white transition-all cursor-pointer justify-center"
                                style={{ minHeight: '44px', minWidth: '40px' }}
                              >
                                <Icons.Plus size={14} strokeWidth={2.5} />
                                <span className="text-[8px] font-bold mt-0.5">更多</span>
                              </button>
                            )}
                            ...
                            {showAllIcons && (
                              <button
                                ...
                                style={{ minHeight: '44px', minWidth: '40px' }}
                              >
                                <Icons.ChevronUp size={14} strokeWidth={2.5} />
                                <span className="text-[8px] font-bold mt-0.5">收起</span>
                              </button>
                            )}
                          </div>
                        </div>
                  ...
                  {paymentMethods && (
                    <div className="space-y-1 animate-fade-in">
                      <label className="text-[10px] font-semibold text-neutral-500 tracking-wider uppercase ml-1">支付渠道</label>
                        ...
                        {paymentMethods.map((method) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-extrabold transition-all border cursor-pointer ${
                                isSelected
                                  ? 'border-black bg-black text-white shadow-xs'
                                  : 'border-neutral-200 bg-white text-neutral-600 hover:border-neutral-350'
                              }`}
                              style={!isSelected ? { backgroundColor: `${method.color}20`, borderColor: `${method.color}80` } : {}}
                            >
