...
export const FixedIncomeModal: React.FC<FixedIncomeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 select-none overflow-x-hidden" id="fixed-income-modal-container">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      <motion.div
        ...
        className="relative w-full max-w-md bg-[var(--color-bg)] border-t-4 border-emerald-500 sm:border-4 sm:rounded-[36px] rounded-t-[36px] p-6 shadow-2xl z-10 overflow-y-auto overflow-x-hidden max-h-[92vh] sm:max-h-[90vh] flex flex-col gap-4"
      >
                  ...
                  {fixedIncomes.map((rule) => {
                    ...
                    return (
                      <div key={rule.id} className="flex items-center gap-3 p-3.5 bg-[var(--color-card)] border border-[var(--color-card-border)] rounded-2xl">
                        <div 
                          className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${catInfo?.bgColor?.startsWith('bg-') ? catInfo.bgColor : 'bg-emerald-100'}`}
                          style={(catInfo?.bgColor && !catInfo.bgColor.startsWith('bg-')) ? { backgroundColor: catInfo.bgColor } : {}}
                        >
                    ...
                    {Object.keys(categories).map((catName) => {
                      ...
                      return (
                        <div key={catName} className="relative">
                          ...
                          >
                            <div 
                              className={`p-1 rounded-xl mb-1 shrink-0 transition-colors ${isDeleteMode ? 'bg-red-100 text-red-600' : isSelected ? 'bg-white/20' : (catInfo.bgColor.startsWith('bg-') ? catInfo.bgColor : '')}`}
                  ...
                  {isAddingCustomCat && onAddCategory && (
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        <div className={showAllIcons ? "flex flex-wrap gap-1.5 max-h-[140px] overflow-y-auto p-1 bg-neutral-50/50 rounded-xl border border-neutral-100" : "flex gap-1.5 overflow-x-auto pb-1"}>
